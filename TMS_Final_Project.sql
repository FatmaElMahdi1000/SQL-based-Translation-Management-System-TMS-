...
CREATE TABLE Language (
    ...
    --No two rows in the Language table can have the same Language_Code.
...
--1- this is, 8B Option is a database design pattern for modeling “inheritance” (supertype/subtype) in relational tables. 
...
--1-Display the Number of assigned jobs in each project
   ...
   --Way #1 
SELECT Project_ID, COUNT(JOB_ID) AS Total_Jobs 
FROM 
(SELECT PROJECT.Project_ID, JOB.Job_ID
FROM PROJECT INNER JOIN JOB
ON JOB.PROJECT_ID = PROJECT.PROJECT_ID) T
GROUP BY PROJECT_ID;

  --Way#2
SELECT Project_ID, count(*) AS Total_Jobs FROM Job
GROUP BY Project_ID;
...
SELECT 
    ...
    sr.Service_Name, --From 'Service Table--
    p.Project_Due_Date AS Deadline, --   deadline  

    pm.Employee_Name AS Project_Manager, --From Internal Emp table
...
LEFT JOIN (     --from Project(Right table) Left Join (total cost of each project - Left table)
                --Project and all of its costs by getting the sum of all project's job assignments 
    SELECT 
        j.Project_ID,
...
--4--Highest Revenue Projects from the project dashboard previously created.
...
---10 ---Projects Assigned to Each Vendor that are in progress: 
--To decide if we can assign them more projects or not
...
-- 11- Employees in all department (NEW)
-- =========================================================================
-- REPORT 1: Active Department Assignments (Inner Join)
-- DESCRIPTION: Displays a list of employees alongside their assigned department.
--              Because this uses an INNER JOIN, it only returns rows where a 
--              match exists on both sides. Employees without a department or 
--              departments without any employees will be excluded.
-- =========================================================================
...
-- =========================================================================
-- REPORT 2: Department Headcount Breakdown (Aggregation with Inner Join)
-- DESCRIPTION: Calculates the total number of employees currently assigned to 
--              each department. Departments with zero employees will not 
--              be shown in this report due to the strict matching of the INNER JOIN.
-- =========================================================================
...
-- =========================================================================
-- DATA STAGING: Creating an Unassigned Department for Join Testing
-- DESCRIPTION: Inserts a new 'Finance' department (ID 6). Since no records 
--              in the EMPLOYEE table point to this ID yet, it acts as the 
--              perfect test subject to observe how Outer Joins handle unmatched data.
-- =========================================================================
INSERT INTO DEPARTMENT(DEPARTMENT_ID, DEPARTMENT_NAME) VALUES (6, 'Finance');


-- =========================================================================
-- REPORT 3: Complete Employee Directory (Left Outer Join)
-- DESCRIPTION: Prioritizes the table on the left (EMPLOYEE). It guarantees that 
--              ALL employees are listed, even if they haven't been assigned 
--              to a department yet. Unassigned employees will show a NULL value 
--              under the DEPARTMENT_NAME column.
-- =========================================================================
SELECT EMPLOYEE_NAME, DEPARTMENT_NAME
...
-- =========================================================================
-- REPORT 4: Complete Department Registry (Right Outer Join)
-- DESCRIPTION: Prioritizes the table on the right (DEPARTMENT). It guarantees that 
--              ALL departments are listed, even if they have zero employees. 
--              Vacant departments (like the newly added 'Finance') will appear 
--              in the results with a NULL value under the EMPLOYEE_NAME column.
-- =========================================================================
--Get all Department rows(Department on the RIGHT, it's been written on the RIGHT) + its matches in the 
--Employees column
SELECT EMPLOYEE_NAME, DEPARTMENT_NAME
FROM EMPLOYEE RIGHT JOIN DEPARTMENT
ON EMPLOYEE.DEPARTMENT_ID = DEPARTMENT.DEPARTMENT_ID;

--Get all employee rows(Employee on the left, it's been written on the left) + its matches in the 
--Department column
SELECT EMPLOYEE_NAME, DEPARTMENT_NAME
FROM EMPLOYEE LEFT JOIN DEPARTMENT
ON EMPLOYEE.DEPARTMENT_ID = DEPARTMENT.DEPARTMENT_ID;

--Selecting FULL ROWS IN BOTH TABLES: 
SELECT EMPLOYEE_NAME, DEPARTMENT_NAME
